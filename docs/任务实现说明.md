...
**文件位置**：`src/core/shell.c` 中的 `shell_loop()` 函数

**实现思路**：
采用扩展要求，实现动态提示符，格式为：`用户名@主机名:当前工作目录$`。其中主机名固定为 `mini-linux`，用户名从当前会话获取，当前工作目录通过 `getcwd()` 动态获取。
...
1. **获取当前工作目录**：
   - 使用标准库函数 `getcwd()` 实时获取进程当前所在的绝对路径。
   - 包含 `<limits.h>` 以使用 `PATH_MAX` 宏定义缓冲区大小。
...
4. **优化显示**：
   - 使用 ANSI 转义序列实现彩色输出（绿色用户名，蓝色目录和提示符）。
   - 使用 `read_line_with_edit` 支持命令行编辑和历史记录回溯。

**示例输出**：
```bash
root@mini-linux:/home/administrator/Mini_computer$ 
```
